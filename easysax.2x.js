...
function EasySAXParser(config) {
  ...
  ;
  var onTextNode = NULL_FUNC,
      onStartNode = NULL_FUNC,
      onEndNode = NULL_FUNC,
      onCDATA = NULL_FUNC,
      onError = NULL_FUNC,
      onComment,
      ...
      onAttention,
      onUnknownNS;
  var is_onComment = false,
      is_onQuestion = false,
      is_onAttention = false,
      is_onUnknownNS = false;
  ...
  this.setup = function (op) {
    for (var name in op) {
      switch (name) {
        case 'entityDecode':
          entityDecode = op.entityDecode || entityDecode;
          break;

        case 'autoEntity':
          isAutoEntity = !!op.autoEntity;
          break;

        case 'defaultNS':
          defaultNS = op.defaultNS || null;
          break;
  ...
  this.on = function (name, cb) {
    ...
    switch (name) {
      case 'startNode':
        onStartNode = cb || NULL_FUNC;
        break;

      case 'endNode':
        onEndNode = cb || NULL_FUNC;
        break;

      case 'text':
      case 'textNode':
        onTextNode = cb || NULL_FUNC;
        break;

      case 'error':
        onError = cb || NULL_FUNC;
        break;

      case 'cdata':
        onCDATA = cb || NULL_FUNC;
        break;
  ...
  function parseNode(indexStart) {
    ...
    if (iE === -1) {
      // не полный xml. дальнейший парсинг бессмыслен
    ...
    while (true) {
      ...
      if (w === 32 || w === 9 || w === 10 || w === 11 || w === 12 || w === 13) {
        // \f\n\r\t\v пробел
    ...
    while (true) {
      ...
      while (w === 32 || w === 9 || w === 10 || w === 11 || w === 12 || w === 13) {
        // \f\n\r\t\v
        w = xml.charCodeAt(++iR);
      }
  ...
  function upNSMATRIX() {
    ...
    for (j = 0; j < nodeParseAttrSize; j += 2) {
      ...
      ;
      value = nodeParseAttrMap[j + 1]; //alias = useNS[isAutoEntity ? value : entityDecode(value)];
  ...
  function parse() {
    ...
    var isTagEnd = false; //var nodeBody;
    ...
    while (indexStartXML !== -1) {
      stopEmit = stopIndexNS > 0; // поиск начала тега
      ...
      if (i === -1) {
        // узел не найден. повторим попытку на след. write
      ...
      if (indexStartXML !== i && !stopEmit) {
        // все что до тега это текст
      ...
      if (w === 33) {
        // 33 == "!"
        var _w = xml.charCodeAt(i + 2); // CDATA
        // ---------------------------------------------


        if (_w === 91 && xml.substr(i + 3, 6) === 'CDATA[') {
          // 91 == "["
        ...
        if (_w === 45 && xml.charCodeAt(i + 3) === 45) {
          // 45 == "-"
      ...
      if (w === 47) {
        ...
        if (indexEndNode === -1) {
          // error  ...> // не нашел знак закрытия тега
        ...
        ;
        isTagStart = false;
        isTagEnd = true; // проверяем что тег должен быть закрыт тот-же что и открывался
        ...
        for (; iQ < indexEndNode; iQ++) {
          var _w2 = xml.charCodeAt(iQ);

          if (_w2 === 32 || _w2 === 9 || _w2 === 10 || _w2 === 11 || _w2 === 12 || _w2 === 13) {
            // \f\n\r\t\v
            continue;
          }
        ...
        ;
        isTagStart = true;
        isTagEnd = xml.charCodeAt(_indexEndNode - 1) === 47;
      ...
      if (isNamespace) {
        if (stopEmit) {
          // потомки неизвестного пространства имен
          if (isTagEnd) {
            if (!isTagStart) {
              if (--stopIndexNS === 0) {
        ...
        if (isTagStart && nodeParseHasNS) {
          // есть подозрение на xmlns //  && (nodeParseAttrResult === null)
          upNSMATRIX();
        }
